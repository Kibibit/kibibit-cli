...
kbRepo.finishGitflowBranch = finishGitflowBranch;
kbRepo.getGitConfig = getGitConfig;
kbRepo.checkoutMainBranch = checkoutMainBranch;
...
kbRepo.getGitConfigUser = getGitConfigUser;
...
function finishGitflowBranch(repo, type, name) {
  if (type === 'feature') {
    return NodeGit.Flow.finishFeature(repo, name);
  }

  if (type === 'hotfix') {
    return NodeGit.Flow.finishHotfix(repo, name);
  }

  if (type === 'release') {
    return NodeGit.Flow.finishRelease(repo, name);
  }

  return Q.fcall(function () {
    throw new KbError(
      'checked out branch is not a branch you can finish',
      1,
      true);
  });
}

function getGitConfig(repo) {
...
function getGitConfigUser(repo) {
  var data = {};
  return repo.config()
    .then(function(config) {
      data.config = config;
      return data.config.getString('user.name');
    })
    .then(function(name) {
      data.name = name;
      return data.config.getString('user.email');
    })
    .then(function(email) {
      data.email = email;
...
function createRepoOrigin(repo, user) {
  ...
  return repo.getRemotes()
    .then(function(allRemotes) {
      data.allRemotes = allRemotes;
      return allRemotes.indexOf('origin') >= 0;
    })
    .then(function(haveOrigin) {
      if (haveOrigin) {
        return;
      }

      if (data.allRemotes.length) {
        return inquirer.prompt([ {
          type: 'list',
          name: 'whereToCreate',
          message: kbString.build([
            'Found remotes but no origins. ',
            'Which remote should be used as the origin?'
          ]),
          choices: [
            data.allRemotes,
            new inquirer.Separator()
          ].concat(['create new remote', 'skip'])
        } ])
          .then(function(answers) {
            if (answers.whereToCreate === 'skip') {
              return;
            }

            if (answers.whereToCreate === 'create new remote') {
              return createNewOriginRepo(repo, user);
            }

            console.log('change origin to this:', answers.whereToCreate);

            return;
          });
      }

      return createNewOriginRepo(repo, user, data);
    });
}

function createNewOriginRepo(repo, user, data) {
  return inquirer.prompt([{
    type: 'list',
    name: 'whereToCreate',
  ...
  },
  {
    ...
    .then(function(remote) {
      data.remote = remote;
      // console.log('successfully added a new remote!',
      // data.remote.name());
...
function checkoutMainBranch(repo, branchName) {
  return NodeGit.Flow.getConfig(repo)
    .then(function(config) {
      return repo.getBranch(config[
        'gitflow.branch.' + branchName
      ] || branchName);
    })
    ...
    .catch(function(error) {
      throw new KbError(error.message, 1, true);
    });
}
