      ...
      if (globalCfg.users.indexOf(answers.selectUser) > -1) {
        ...
        return GLOBAL.repo.config()
          .then(function(config) {
            ...
            return GLOBAL.config.setString('user.name', GLOBAL.username);
          })
          .then(function() {
          ...
          .then(function(githubUser) {
            return GLOBAL.config.setString('user.email', githubUser.email);
          });
      } else {
...
function associateUser() {
  ...
  if (globalCfg.users && globalCfg.users.length > 0) {
    ...
    runUserProcess = selectExistingUser;
  } else {
    console.log('no registered users found');

    globalCfg.users = [];

    runUserProcess = loginNewUser;
  }

  return runUserProcess();
}
